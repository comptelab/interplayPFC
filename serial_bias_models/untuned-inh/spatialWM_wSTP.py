...
i_cue_amp     = 1.2*0.25*0.25*nA            # cue current amplitude
...
t_delay	      = 2000*ms		# delay duration

fact = 1.42#1.45 #1.275#1.28
...
simtime     = sec_cue_stop + t_delay #reset_stop     # total simulation time [ms] 
...
g_ext_e	    = 0.9*3.1*nS#0.95*3.1*nS  		# [nS] maximum conductance from external to pyramidal cells
g_ext_i	    = 2.38*nS 		# [nS] maximum conductance from external to interneuron cells
Gee_ampa    = 2.*0.251*nS  #0.251*nS #0.391*nS      # [nS] maximum conductance from pyramidal to pyramidal cells
...
Gee	    = fact*0.35*nS #0.381*nS    #0.274*nS      # [ns] synaptic conductance from pyramidal to pyramidal cells
...
Pe.x = 0
Pe.i_e = 0
Pe.synu=t_su*U*r0/(1+t_su*U*r0)
Pe.synx=1/(1+t_sx*r0*Pe.synu)
...
##selfnmda=IdentityConnection(Pe, Pe, 'x', weight=1.0, modulation='synf')
#selfnmda = Connection(Pe,Pe,'x',weight=1.0, modulation='synf')
#selfnmda = np.identity(NE)
...
#stp=STP(Cee_ampa,taud=200*ms,tauf=20*ms,U=0.45)
# set u to the st
...
i,t = spikes.it
plot(t, i, 'k.')
fill_between([tc_start,tc_start+tc_dur],[0,0],[NE,NE],alpha=0.2,color="gray")
xlim(0,max(t))
ylim(0,NE)
...
w2=500*ms
n_wns = (simtime-w2)/w1
fr2 = zeros([NE,int(n_wns)])

for ti in range(int(n_wns)):
     t_beg = ti*w1
     t_end = t_beg + w2
     for n in range(NE):
...
subplot(3,1,2)
n = int(i_cue_ang/360.0*NE)
plot(linspace(0,simtime,len(u_m[n])),mean(u_m[n-10:n+10],0),label="facilitation at bump");
plot(linspace(0,simtime,len(x_m[n])),mean(x_m[n-10:n+10],0),label="depression at bump");
fill_between([tc_start,tc_start+tc_dur],[0,0],[1,1],alpha=0.2,color="gray")
fill_between([sec_cue_start,sec_cue_start+tc_dur],[0,0],[1,1],alpha=0.2,color="gray")

legend()

subplot(3,1,3)
plot(linspace(0,simtime,len(fr2[n])),mean(fr2[n-10:n+10,:],0),label="firing rate")
fill_between([tc_start,tc_start+tc_dur],[0,0],[max(concatenate(fr2)),max(concatenate(fr2))],alpha=0.2,color="gray")
