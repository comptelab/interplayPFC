...
print(stim, probe, resp)
if ((stim[:9] != resp[:9]) | (stim[:9] != probe[:9])): 
    print('files do not match'); kill  # make sure subjects match

nIter           = 10          # iterations of random sampling training set
window          = 50          # number of samples for downsampling (s.r. 512 Hz)
...
def crossmatrix_IEM(U_train, M_train, U_test, bin_test):
    M_pred  = np.zeros([M_train.shape[0], U_test.shape[1], U_test.shape[1]])
...
def run_trialwise(i):
    print(i)
    tf      = np.zeros([nTrials,nBins,nSamps,nSamps])
    ...
    for trial in range(nTrials):
        ...
        tf[trial,:,:,:] = crossmatrix_IEM(U_train, M_train, U_test, bin_test)
...
tf = np.array([r for r in results])
tf_mean = np.mean(tf,0)
print(np.shape(tf_mean))

# save
name    = 'crossmatrix_'  + '_iter-%.0f_' % nIter
...
# np.save('../../Data/decoders_and_behavior_EEG/crosstime', time)

del data; del info; del time